...
public class backRoadMark : MonoBehaviour
{
    // 判断是否碰撞到路标
    ...
    public void FishingRoadMark()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E))
        {
           // showDialog = !showDialog;
            ...
            // 变更文本
            string[] textContent = { "前往火车站" };

            dialogGameObject.transform.GetChild(0).GetComponent<DialogPrefab>().message = textContent;
            //dialogGameObject.transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<Text>().text = "前往火车站";
